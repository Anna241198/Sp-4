...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	runEnergy := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runEnergy
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meenSpeedInMsec := meanSpeed(action, duration) * kmhInMsec
	walkEnergy := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meenSpeedInMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return walkEnergy
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimmEnergy := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimmEnergy
}
